...
for i in range(len(test)):
    ...
    """for j in range(len(test[i])-1):
        try:
            assert set([part for part in divide_sentence(test[i][j+1]) if part != '|']) in [set(n for n in divide_sentence(m[0]) if n != '|') for m in x.CNF], f'Failed on test {i+1}. Expected CNF form not found in belief base. Added belief: {test[i][0]}. Expected CNF part: {test[i][j+1]}'
